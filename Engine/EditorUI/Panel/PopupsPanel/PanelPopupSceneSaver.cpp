...
PanelPopupSceneSaver::PanelPopupSceneSaver()
{
	opened = false;
	enabled = true;
...
void PanelPopupSceneSaver::Render()
{
	if (popup_shown)
	{
		popup_shown = false;
...
void PanelPopupSceneSaver::RenderAccessPath()
{
...
void PanelPopupSceneSaver::RenderCurrentFolderContents()
{
...
void PanelPopupSceneSaver::PushCurrentPath(Path* new_current_path)
{
...
void PanelPopupSceneSaver::PopCurrentPath(Path* new_current_path)
{
	current_path = new_current_path;
...
std::string PanelPopupSceneSaver::GetNormalizedPath()
{
...
void PanelPopupSceneSaver::SetPopupSelection()
{
	has_selected = true;
	ImGui::CloseCurrentPopup();
...
float PanelPopupSceneSaver::GetFrameHeightWithSpacing()
{
	ImGuiContext& g = *GImGui;
	return g.FontSize + g.Style.FramePadding.y * 2.0f + g.Style.ItemSpacing.y;
}

bool PanelPopupSceneSaver::HasSelected() const
{
	return has_selected;
}

std::string PanelPopupSceneSaver::GetSelected()
{
	has_selected = false;
