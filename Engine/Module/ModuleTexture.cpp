...
{
	std::pair<bool, std::string> imported = App->texture_importer->Import(File(texture_path));
	if (!imported.first)
	{
		return nullptr;
	}

	return App->texture_importer->Load(imported.second);
}
...
GLuint ModuleTexture::LoadCubemap(const std::vector<std::string> & faces_paths) const
{
	...
	{
		std::string ol_texture = App->texture_importer->Import(File(faces_paths[i])).second;
		faces_paths_dds.push_back(ol_texture);
	}
	return static_cast<GLuint>(App->texture_importer->LoadCubemap(faces_paths_dds));
}
