...
{
	float4x4 model_global_matrix_transposed = App->editor->selected_game_object->GetTransform()->GetGlobalModelMatrix().Transposed();
	...
	if (!gizmo_released && !App->actions->clicked)
	{
		...
		case ImGuizmo::TRANSLATE:
			App->actions->previous_transform = App->editor->selected_game_object->GetTransform()->GetTranslation();
			break;
		case ImGuizmo::ROTATE:
			App->actions->previous_transform = App->editor->selected_game_object->GetTransform()->GetRotationRadiants();
			break;
		case ImGuizmo::SCALE:
			App->actions->previous_transform = App->editor->selected_game_object->GetTransform()->GetScale();
			break;
	...
	if (ImGuizmo::IsUsing())
	{
		...
		App->editor->selected_game_object->GetTransform()->SetGlobalModelMatrix(model_global_matrix_transposed.Transposed());
	}
