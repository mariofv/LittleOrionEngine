...
uint32_t MaterialImporter::ImportMaterialTexture(const std::string& texture_described_path, const Path& material_file_folder_path) const
{
	...
	APP_LOG_INFO("Loading material texture in described path %s.", sanitazed_string.c_str());
	if (App->filesystem->Exists(sanitazed_string))
	{
		APP_LOG_INFO("Material loaded correctly from %s.", sanitazed_string.c_str());
	...
	APP_LOG_INFO("Loading material texture in model folder path %s.", material_file_folder_path.GetFullPath().c_str());
	if (App->filesystem->Exists(textures_path))
	{
		APP_LOG_INFO("Material loaded correctly from %s.", textures_path.c_str());
	...
	APP_LOG_INFO("Loading material texture in textures folder %s.", textures_path.c_str());
	if (App->filesystem->Exists(textures_path))
	{
		APP_LOG_INFO("Material loaded correctly from %s.", textures_path.c_str());
		return App->resources->InternalImport(*App->filesystem->GetPath(textures_path));
