...
void MaterialImporter::ImportMaterialFromMesh(
	const aiScene* scene,
	size_t mesh_index, 
...
) const
{
	...
	{
		Material imported_material(material_assets_file_path);
		...
		App->filesystem->Save(material_assets_file_path, serialized_scene_string.c_str(), serialized_scene_string.size() + 1);
	}

	
	App->filesystem->Copy(material_assets_file_path, material_library_file_path);
}
