...
{
	std::vector<std::shared_ptr<File>> resource_already_in_library;
	App->filesystem->GetAllFilesInPath(path_to_look_in, resource_already_in_library);

	//Check if the resource is already loaded
	auto it = std::find_if(resource_already_in_library.begin(), resource_already_in_library.end(), [file_to_look_for](const std::shared_ptr<File> & file)
	{
