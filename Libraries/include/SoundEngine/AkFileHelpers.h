...
#else
		const int size = ::WideCharToMultiByte(CP_UTF8, 0, in_pszBasePath, -1, NULL, 0, 0, NULL);

		std::vector<char> utf16ToUTF8(size);
		::WideCharToMultiByte(CP_UTF8, 0, in_pszBasePath, -1, &utf16ToUTF8[0], size, 0, NULL);

		fileAttributes = GetFileAttributes(utf16ToUTF8.data());
#endif
